...
async def test_predict(files: list[UploadFile],dpi: int = Query (217, description="dpi数值"),adjust: int = Query  ( 0,description="对比度和降噪开关,1打开 0关闭")):
    """
    ...
    """
    comm.adjust_init=adjust
    pdf2img.dpi_init=dpi
    final_files = []
    for file in files:
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        ...
        for file in final_files:
            ...
            if file['type'] == '.pdf':
                ...
                test_img=comm.byte_to_img(file_contents)
                height, width = test_img.shape[:2]
                new_size = (int(width / 1.157), int(height/1.157))
                change_img = cv2.resize(test_img, new_size, interpolation=cv2.INTER_LINEAR)
                img,rgb_img=comm.process_image(change_img)
                #img, rgb_img = comm.process_image(comm.byte_to_img(file_contents))
                imgs.append(img)
                rgb_imgs.append(rgb_img)
